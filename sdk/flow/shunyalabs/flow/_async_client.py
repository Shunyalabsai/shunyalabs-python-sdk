...
class AsyncClient(EventEmitter):
    """
    High-level asynchronous client for the Shunyalabs Flow API.
    ...
    def __init__(
        ...
        *,
        ...
        auth
            ...
            the ``SHUNYALABS_API_KEY`` environment variable.
        api_key
        ...
        if auth is None:
            if not api_key and not os.getenv("SHUNYALABS_API_KEY"):
                raise ConfigurationError(
                    "No authentication provided.  Supply `auth`, `api_key` or "
                    "set the SHUNYALABS_API_KEY environment variable."
                )
