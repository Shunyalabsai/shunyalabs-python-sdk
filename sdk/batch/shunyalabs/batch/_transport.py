"""
Transport layer for Shunyalabs Batch HTTP communication.

This module provides the Transport class that handles low-level HTTP
communication with the Shunyalabs Batch API, including connection management,
...
class Transport:
    """
    HTTP transport layer for Shunyalabs Batch API communication.

    This class handles all low-level HTTP communication with the Shunyalabs
    Batch API, including connection management, request serialization,
    authentication, and response handling.

    Args:
        url: Base URL for the Shunyalabs Batch API.
    ...
    async def _prepare_headers(self) -> dict[str, str]:
        ...
        auth_headers["User-Agent"] = (
            f"shunyalabs-batch-v{get_version()} python/{sys.version_info.major}.{sys.version_info.minor}"
        )
    ...
    async def _handle_response(self, response: aiohttp.ClientResponse) -> dict[str, Any]:
        ...
        try:
            ...
            if (
                ...
                or response.content_type == "application/vnd.Shunyalabs.v2+json"
            ):
