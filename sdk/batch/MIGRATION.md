...
This guide helps users migrate from the legacy Shunyalabs Batch Client (`Shunyalabs-python`) to the new Shunyalabs Batch SDK (`shunyalabs-batch`). The new SDK provides a modern async API, improved error handling, and enhanced authentication options with minimal dependencies.
...
- **Import paths**: `shunyalabs.batch_client` → `shunyalabs.batch`
...
```python
from shunyalabs.models import ConnectionSettings, BatchTranscriptionConfig
from shunyalabs.batch_client import BatchClient
...
```python
from shunyalabs.batch import AsyncClient, FormatType, JobConfig, JobType, TranscriptionConfig

async with AsyncClient(os.environ.get("SHUNYALABS_API_KEY")) as client:
    config = JobConfig(
