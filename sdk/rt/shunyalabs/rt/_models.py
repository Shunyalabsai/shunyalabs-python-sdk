...
class AudioEncoding(str, Enum):
    ...
    The Shunyalabs RT API supports several audio encoding formats for
...
class ClientMessageType(str, Enum):
    ...
    client can send to the Shunyalabs RT API during a transcription session.
...
class ServerMessageType(str, Enum):
    ...
    These enum values represent the different types of messages that the
    Shunyalabs RT API can send to the client.
...
class TranscriptResult:
    ...
    from the Shunyalabs RT API, extracting and organizing the relevant
    information from server messages.

    Attributes:
        format: Shunyalabs JSON output format version number.
    ...
    def from_message(cls, message: dict[str, Any]) -> TranscriptResult:
        ...
        Args:
            message: Raw message dictionary from the Shunyalabs RT API
