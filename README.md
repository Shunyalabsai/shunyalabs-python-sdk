...
Control the emotional tone by passing a `style` tag in the text prefix (e.g. `"<Happy> Hello!"`).
...
config = TTSConfig(model="zero-indic", voice="Varun")
result = await client.tts.synthesize("<Neutral> Your account balance is five thousand rupees.", config=config)
```
