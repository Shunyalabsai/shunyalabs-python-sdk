# Makefile for Shunyalabs Python SDKs
...
format-rt:
	cd sdk/rt/shunyalabs && black .
	cd sdk/rt/shunyalabs && ruff check --fix .

format-batch:
	cd sdk/batch/shunyalabs && black .
	cd sdk/batch/shunyalabs && ruff check --fix .

format-flow:
	cd sdk/flow/shunyalabs && black .
	cd sdk/flow/shunyalabs && ruff check --fix .

format-tts:
	cd sdk/tts/shunyalabs && black .
	cd sdk/tts/shunyalabs && ruff check --fix .

# Linting targets
lint-all: lint-rt lint-batch lint-flow lint-tts

lint-rt:
	cd sdk/rt/shunyalabs && ruff check .

lint-batch:
	cd sdk/batch/shunyalabs && ruff check .

lint-flow:
	cd sdk/flow/shunyalabs && ruff check .

lint-tts:
	cd sdk/tts/shunyalabs && ruff check .

# Type checking targets
type-check-all: type-check-rt type-check-batch type-check-flow type-check-tts

type-check-rt:
	cd sdk/rt/shunyalabs && mypy .

type-check-batch:
	cd sdk/batch/shunyalabs && mypy .

type-check-flow:
	cd sdk/flow/shunyalabs && mypy .

type-check-tts:
	cd sdk/tts/shunyalabs && mypy .
